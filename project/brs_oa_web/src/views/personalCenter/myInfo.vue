  ...
  <div class="profile-page">
    ...
    <div class="profile-base-wrapper">
      <div class="profile-base">
        ...
        <div class="profile-base">
          <img class="person-head" :src="personalDetailInfo.picture">
          <div class="person-base">
            <p>
              <span>{{ personalDetailInfo.name }}</span>
              <span>{{ personalDetailInfo.gender===0?'男':'女' }}</span>
              <span>{{ personalDetailInfo.politicsStatus }}</span>
              <span>{{ personalDetailInfo.nation }}</span>
            </p>
            <p>
              <span>工号：{{ personalDetailInfo.empNo }}</span>
              <span>部门：{{ personalDetailInfo.deptName }}</span>
              <span>入职日期：{{ personalDetailInfo.hireDate }}</span>
            </p>
            <p>
              <span>手机：{{ personalDetailInfo.mobile }}</span>
              <span>邮箱：{{ personalDetailInfo.email }}</span>
            </p>
          </div>
        </div>
      </div>
        ...
        <div>
          <ul class="per-info-ul">
            ...
            <li>
              ...
              <span>本地紧急联系人电话：{{ personalDetailInfo.emergencyContactPhone }}</span>
            </li>
          </ul>
        </div>
      </div>
...
<script>
import { getPersonalDetailInfo } from "@/api/staff";
export default {
  name: "myInfo",
  data() {
    return {
