<template>
	<div class="attendance-apply-list">
			...
			style="width: 100%">
				...
				<el-table-column label="操作" width="160" prop="approvalStatus">
					<template slot-scope="scope">	
						...
						<el-button type="success" size="small" v-if="scope.row.approvalStatus==0"  @click="editRow(scope.$index,scope.row.serialNumber)">添加材料</el-button>
		...
		<el-dialog title="新增申请" :visible.sync="workflowInsert" :close="resetForm()">
			<workflow-apply-form ref=addFormBox @callback="submitCallback"></workflow-apply-form>
		</el-dialog>
		...
		<el-dialog title="修改申请" :visible.sync="workflowEdit.workflowEditBoxStatus">
			<workflow-edit  :workflowInfo="list[workflowEdit.editIndex]" :serialNumber="workflowEdit.serialNumber" @callback="submitCallback"></workflow-edit>
		</el-dialog>
...
export default {
	components:{
		pagination,workflowApplyForm,workflowPreview,workflowEdit
	},
	data(){
		return {
			// config
