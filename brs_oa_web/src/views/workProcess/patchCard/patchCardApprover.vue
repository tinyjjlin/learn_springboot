<template>
  <div class="patchcard-approval-list">
    ...
    <el-dialog title="预览申请" :visible.sync="patchCardPreview.patchCardPreviewBoxStatus">
      <patch-card-apply-preview
        :serialNumber="patchCardPreview.serialNumber"
        :workflowInfo="list[patchCardPreview.previewIndex]"
      ></patch-card-apply-preview>
    </el-dialog>
  </div>
</template>
...
import patchCardApplyPreview from "../components/patchCardApplyPreview";

import {
  getCardWorkflowApprover,
  getAttendanceApplyTypes,
  updateCardWorkflowApproval
} from "@/api/workProcess";
export default {
  components: {
    pagination,
    patchCardApplyPreview
  },
  data() {
    return {
      // config
      tableHead: [
        ...
        { prop: "reissueCardTime", label: "补卡时间", width: 170 },
      ...
      patchCardPreview: {
        patchCardPreviewBoxStatus: false,
        previewIndex: "",
        serialNumber: ""
      }
    };
  ...
  methods: {
    initView: function() {
      ...
      getCardWorkflowApprover(this.query)
        .then(res => {
    ...
    updateWorkflowState(row, value) {
      ...
      this.$confirm("你确定要" + value + "吗?", "提示", {
        ...
        updateCardWorkflowApproval(data)
          .then(res => {
            if (res.data.code == 200) {
    ...
    previewRow: function(index, serialNumber) {
      this.patchCardPreview.serialNumber = serialNumber;
      this.patchCardPreview.previewIndex = index;
      this.patchCardPreview.patchCardPreviewBoxStatus = true;
    }
  }
};
</script>
