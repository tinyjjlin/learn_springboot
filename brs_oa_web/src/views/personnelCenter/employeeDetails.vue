<template>
  <div class="emp-container">
    <el-select
      ...
      clearable
      filterable
      @change="departmentThis(department)"
      placeholder="请选择部门"
    >
      <el-option
      ...
      <div class="emp-list-wrapper">
        <ul class="emp-list clearfix">
          <li class="item" :key="index" v-for="(staff,index) in list">
            <div class="emp-info" @click="details(staff.empNo)">
    ...
    <el-dialog title="查看员工个人信息" :visible.sync="dialogFormVisible">
      <my-info :empNo="empNo" ref="myinfo"></my-info>
    </el-dialog>
  </div>
</template>
...
import myInfo from "@/views/personnelCenter/attendanceMng/myInfo";

export default {
  name: "staffList",
  components: { Pagination, "my-info": myInfo },
  data() {
    return {
      list: null,
      total: 0,
  ...
  methods: {
    ...
    details(index) {
      ...
      this.empNo = index;
      this.dialogClickCount++;
      if (this.dialogClickCount > 1) {
        this.$refs.myinfo.showPresonalInfo(index);
      }
    }
  }
};
